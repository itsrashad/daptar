...
function daptar_breadcrumbs ($args = array())
{
    if (is_front_page())
    {
        return;
    ...
    $defaults  = array(
        ...
        'home_title'          => esc_html__('Home ', 'daptar')
    );

    $args      = apply_filters( 'daptar_breadcrumbs_args', wp_parse_args( $args, $defaults ) );
    ...
    $html .= '<div class="container"><span class="item-home"><a class="bread-link bread-home" href="' . esc_url(get_home_url()) . '" title="' . esc_attr( $args['home_title'] ) . '">' .  esc_html($args['home_title']) . '</a></span>';
    $html .= $separator;
    ...
    elseif (is_search())
    {
        $html .= '<span class="item-current item-search"><span class="bread-current bread-search">'.__('Search results for:', 'daptar').' ' . get_search_query() . '</span></span>';
    }
    elseif (is_404())
    {
        $html .= '<span>' . __('Error 404', 'daptar') . '</span>';
    }
    elseif (is_home())
    {
    ...
    $html .= '</div></div>';
    $html = apply_filters('daptar_breadcrumbs_filter', $html);
